...
from network_functions.ba_solver import admm_ba
import torch
...
class InitModel(nn.Module):
    ...
    def __init__(self, dV, dS, n_factormers, admm_iters=5, device='cpu',
        ...
        self.factormer_VS = FactormerLayer(d=dV, edge_dim=edge_dim, **fk_kwargs)
        self.factormer_SV = FactormerLayer(d=dS, edge_dim=edge_dim, **fk_kwargs)

        # Weight initialization
        self.apply(_init_weights_xavier)

    def forward(self, V0, S0, edge_index, edge_attr, M, obs_matrix, admm_iters_override = None):
        """
        ...
        P = self.extract_V(V).view(-1, 3, 4)
        X = self.extract_S(S)
        ones = torch.ones(X.shape[0], 1, device=X.device, dtype=X.dtype)
        X = torch.cat([X, ones], dim=1)

        admm_iters = self.admm_iters if admm_iters_override is None else admm_iters_override

        # Optional: run ADMM bundle adjustment
        P_seq, X_seq = admm_ba(P,X, M, obs_matrix, 1, admm_iters, 1, 1, 1e-2, True)
