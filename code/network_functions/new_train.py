...
def train_model(model, train_dataloader, val_dataloader, optimizer, scheduler, loss_fn,
                epochs, best_px_error, train_M_gt_list, val_Ns_list, val_M_gt_list, scene_type='Projective', device='cpu', warmup_epochs=5,
    ...
    for epoch in range(epochs):
        ...
        print(f"train loss={avg_train_loss:.6f}  val loss={avg_val_loss:.6f}  px_error={px_error}  (admm_iters={solver_iters})")
        ...
        torch.save({
            ...
            'px_error': px_error
        }, '../../pretrained_models/last_model.pth')
        ...
        if px_error < best_px_error:
            ...
            print(f"Best model saved with px_error={px_error:.6f}")
    return best_px_error