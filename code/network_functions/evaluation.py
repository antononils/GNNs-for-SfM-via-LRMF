...
def evaluate_model(dataloader,Ns_list,model_path,model,device='cpu'):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            # Forward pass
            P_seq, X_seq = model(V0, S0, edge_index, edge_attr, M, obs_matrix, 0)
            P_final = P_seq[-1]
            X_final = X_seq[-1]
            # Compute pixel error
            px_error = compute_pixel_error(P_final, X_final, M, Ns, obs_matrix)
    ...
    return px_errors, P_finals, X_finals, Ms, obs_matrices
