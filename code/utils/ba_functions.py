...
def euc_ba(xs, Rs, ts, Ks, Xs_our=None, Ps=None, Ns=None, repeat=True, triangulation=False, return_repro=True, print_out=True):
    ...
    if repeat:
        if return_repro:
            results['repro_middle3'] = np.nanmean(geo_utils.reprojection_error_with_points(new_Ps, new_Xs, xs, visible_points))
        ...
        if return_repro:
            results['repro_middle3_triangulated'] = np.nanmean(geo_utils.reprojection_error_with_points(new_Ps, new_Xs, xs, visible_points))

        # second ba with triangulated x
        new_Rs, new_ts, new_Ps, new_Xs, converged = ceres_utils.run_euclidean_python_ceres(new_Xs, visible_xs, new_Rs, new_ts, Ks, point_indices, print_out=print_out)
        results['converged4'] = converged
